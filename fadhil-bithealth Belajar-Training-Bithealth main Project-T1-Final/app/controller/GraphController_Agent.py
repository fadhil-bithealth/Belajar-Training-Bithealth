...
def detection_condition(state):
    if state["UnifiedResult"].is_anomaly:
        return "output"
    else:
...
graph.add_node("primary_check_detection", unified_node)
graph.add_node("batch_and_expiry", batch_and_expiry_node)
graph.add_node("quantity", quantity_node)
graph.add_node("output", output_node)

graph.set_entry_point("primary_check_detection")

# --- CONDICIONAL FLOW ---
graph.add_conditional_edges(
    "primary_check_detection",
    detection_condition
)
